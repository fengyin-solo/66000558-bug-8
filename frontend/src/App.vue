<template>
  <div class="app-root">
    <header class="top-bar">
      ...
      <div class="status"><span class="dot" :class="{on:store.wsConnected,pending:store.wsStatus==='connecting'}"></span>{{ store.wsStatus==='open' ? '实时' : store.wsStatus==='connecting' ? '连接中…' : '已断开' }}</div>
    </header>
    <div class="main-grid">
      <div class="col-wide">
        <OrderBookDepth />
...
.dot{width:8px;height:8px;border-radius:50%;background:#ef4444}.dot.pending{background:#f59e0b}.dot.on{background:#22c55e}
